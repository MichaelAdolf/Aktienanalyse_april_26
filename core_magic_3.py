...
# Kontext-Archetypen (finalisiert):
_CONTEXT_PATTERNS = {
    "STRATEGIC_WIN": [
    ...
    try:
        ...
        for item in channel.findall("item"):
            ...
            if not keep:
                continue
            if title:
                out.append({"title": title, "description": desc, "url": link, "source": src, "published_at": pub})
        seen = set()
        dedup = []
        for it in out:
            t = it.get("title", "")
...
def lade_aktien_stimmung(symbol: str, days: int = 7, limit: int = 12) -> dict:
    ...
    if not items:
        return {"sentiment": "NEUTRAL", "contexts": [], "explanation": "Keine aktuellen News gefunden oder Quelle aktuell nicht erreichbar.",
                "pos_hits": 0, "neg_hits": 0, "headlines": [], "as_of": as_of}
    ...
    return {"sentiment": sentiment, "contexts": contexts, "explanation": explanation, "pos_hits": pos_hits,
            "neg_hits": neg_hits, "headlines": items, "as_of": as_of}
